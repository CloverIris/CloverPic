...
現在活発に開発中で、基本キャンバスエンジン、完全なレイヤーシステム（テキストレイヤー含む）、ブラシエンジン、デュアルスタックのタブレットサポート、選択ツール、変形ツール、シェイプツール、テキストツール、フィルターシステム、ファイル I/O が既に実装されています。
...
- **プロフェッショナルなレイヤーシステム**：18 種類のブレンドモードに加え、不透明度、表示/非表示、ロック、透明度保護、Solo モードが可能。
- **レイヤー多態アーキテクチャ**：`RasterLayer`（ピクセル描画）+ `TextLayer`（編集可能なテキスト、DirectWrite ラスタライズキャッシュ）
- **ブラシエンジン**：5 種類のティップ（RoundHard、RoundSoft、Flat、Bristle、Texture）をサポート。flow、wetMix、spacing、筆圧マッピングに対応。
- **完全なツールセット**：ブラシ、消しゴム、スポイト、ペイントバケツ、グラデーション、移動、套索/矩形/楕円/魔棒選択、変形、テキスト、シェイプ
- **デュアルスタックのタブレットサポート**：Windows Ink API（優先）→ WinTab API（フォールバック）→ Mouse。筆圧、傾き、回転を完全にサポート。
- **フィルターシステム**：6 つの破壊的フィルター（明度/コントラスト、色相/彩度、ガウスぼかし、鮮明化、反転、閾値）+ FilterDialog パラメータダイアログ
- **ファイル I/O**：`.vvp` カスタムバイナリ形式（VVP v1/v2 後方互換）+ WIC PNG エクスポート
- **Undo/Redo**：`HistoryManager` + `StrokeUndoItem` デュアルスナップショット機構、50 ステップ上限
- **メモリ認識システム**：キャンバス作成前に安全なメモリ予算を自動計算し、緑/黄/赤のダッシュボードインジケータで表示。
- **ダークテーマ UI**：DPI 対応スケーリング（`Theme::Scale`）で、100%～200% の DPI 設定に対応。
- **最近使用ファイル**：`RecentFilesManager` が自動的に最近開いた 10 ファイルを記録。
...
| テキスト | DirectWrite（システムフォント列挙 + テキストラスタライズ） |
...
│   ├── Core/         # Project, Layer(基底クラス), RasterLayer, TextLayer, LayerManager, BlendMode, MemoryAdvisor, SelectionMask, Filters, History, ProjectIO, RecentFilesManager
...
│   ├── UI/
│   │   ├── Core/     # Window, Theme, ToolType, メッセージ配信基底クラス
...
| M3 | ✅ | TileGrid レイヤーシステム、18 種類のブレンドモード、パネル、ショートカット |
| M4 | ✅ | 完全なブラシエンジン、5 種類のティップ、BrushPanel、WinTab サポート |
| M5 | ✅ | 選択ツール、変形、塗りつぶし、グラデーション、テキストツール、ナビゲーター、Solo モード、ToolBar 同期 |
| M6 | 🚧 | ファイル I/O（.vvp v1/v2 / PNG）、履歴/Undo、フィルター、テキストレイヤー保存 |
