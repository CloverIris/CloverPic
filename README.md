...
目前处于活跃开发阶段，已实现基础画布引擎、图层系统、笔刷引擎与数位板双栈支持。

## 核心特性

- **纯原生实现**：基于 Win32 API 与 Direct2D，无 Qt/Electron 等外部 UI 框架依赖
- **自研渲染管线**：CPU 软合成 + D2D 硬显示，256×256 TileGrid 分块内存管理，支持 Copy-on-Write
- **专业图层系统**：8 种混合模式（Normal / Multiply / Screen / Overlay / Difference / Add / Subtract / Darken / Lighten），支持不透明度与可见性控制
...
| M3 | ✅ | TileGrid 图层系统、8 种混合模式、面板布局、快捷键 |
| M4 | ✅ | 完整笔刷引擎、5 种笔头、BrushPanel、WinTab 支持 |
| M5 | 🚧 | 选择工具、变换、填充、文字工具、导航器 |
| M6 | ⏳ | 文件 I/O（.vvp / PNG / PSD）、历史/撤销、滤镜 |
| M7 | ⏳ | 云端同步、延时摄影、导出功能 |
| M8 | ⏳ | 国际化、设置面板、性能优化 |
