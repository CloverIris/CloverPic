#include "Core/Services/MemoryAdvisor.h"
...
uint64_t MemoryAdvisor::GetTotalPhysicalMemory() const {
    if (auto* provider = CoreServices::GetMemoryInfoProvider()) {
        return provider->GetTotalPhysicalMemory();
    }
    return 0;
}
...
uint64_t MemoryAdvisor::GetAvailablePhysicalMemory() const {
    if (auto* provider = CoreServices::GetMemoryInfoProvider()) {
        return provider->GetAvailablePhysicalMemory();
    }
    return 0;
}
