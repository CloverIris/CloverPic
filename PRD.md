...
- **项目文件格式**：`.vvp` (ViVidPic Project)，实质为带元数据的 ZIP 包，内含：
  - `layers/` - 图层数据（分文件存储，支持增量保存）
  - `thumb.png` - 项目缩略图
  - `meta.json` - 画布尺寸、DPI、颜色模式、创建时间等
  - `history/` - 操作历史（可选，用于撤销恢复）
  - `timelapse/` - 延时摄影帧序列
...
**图层面板（图层）**
- 图层列表（缩略图 + 名称 + 可见性/锁定图标）
- 不透明度滑块
- 混合模式下拉框（正常、正片叠底、滤色、叠加、差值等 20+ 种）
- 复选框：保护透明度、图层剪贴、锁定
- 底部工具栏：新建图层、删除图层、合并、新建组、添加蒙版
...
| 灰度图层 | 仅灰度通道，用于起稿 |
| 透明图层 | 仅用于组织，无像素数据 |
| 图层组 | 可嵌套，支持组级混合模式与不透明度 |
| 文字图层 | 可编辑文本（保留字体、字号信息） |
| 矢量图层 | 钢笔路径，可无损缩放（用于漫画框线） |
| 调整图层 | 非破坏性色彩调整（色相/饱和度/曲线） |

#### 4.5.2 图层操作
- **基础**：新建、删除、复制、合并（向下/可见/所选）、拼合图像
- **可见性**：显示/隐藏、独奏（仅显示当前图层）
- **锁定**：透明像素锁定、全部锁定、位置锁定
- **蒙版**：图层蒙版（256级灰度）、剪贴蒙版（基于下方图层Alpha）
- **混合模式**：至少支持 20 种标准混合模式
...
| **M5** | 2周 | 选择工具、变换、填充、文字工具、导航器 |
| **M6** | 2周 | 文件 I/O（VVP/PNG/PSD）、历史/撤销、滤镜 |
